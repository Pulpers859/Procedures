...
Ground truth: procedural access geometry, not a routine TTE teaching image. Oblique torso/procedure view with xiphoid/costal margin, low subxiphoid probe, and needle entering adjacent to the probe. Needle path is the dominant teaching element: shallow, under the costal margin, toward the patient's left shoulder, with the needle tip approaching the pericardial effusion. Myocardium and liver are visible as danger anatomy so a steep/deep path reads as unsafe. A small ultrasound inset is acceptable only as confirmation.
Auto-fail: image reads primarily as a TTE/subxiphoid probe view rather than the procedure; needle is visually secondary; no liver danger anatomy; no myocardium danger anatomy; steep/deep trajectory appears acceptable; needle starts inside the heart; arrow loops/curls/points downward out of the body; title/caption/subtitle text.
