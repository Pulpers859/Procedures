...
- **Patient laterality or view orientation is mirrored** so danger anatomy appears on the wrong anatomical side.
...
- **Unexplained tool/device anatomy** that changes the procedural meaning (e.g., a random catheter, guidewire, sheath, or tube crossing the target organ).
...
1. **Labels are optional.** If an asset has no `requiredLabels`, any in-image text is a defect unless the per-image answer key explicitly allows it. The image must teach through composition, arrows, target marks, color, and anatomy.
2. **Labels are nouns, never instructions.** Each label names the anatomical structure or single target its leader line touches. No imperative or warning text ("cut away from globe", "avoid", "do not", "danger"). Direction and motion are shown with arrows, not words. A directive label is a defect: delete it, don't reword it.
3. **Correct placement beats symmetry.** A label sits on the side nearest its structure and its leader line lands exactly on that structure. Never force labels into evenly balanced two-column layouts — that forced balance previously dragged the "Lateral canthus" label onto the wrong (medial) corner. An asymmetric layout with correct leaders passes; a symmetric layout with one misplaced leader fails.
4. **Required labels exactly once.** Every label in the asset's `requiredLabels` must appear once, spelled verbatim. Duplicated labels, misspellings, parenthetical expansions, invented captions, titles, subtitles, body text, organ/basic-anatomy labels not required by the asset, direction labels, path labels, or other extra labels are failures, even if the anatomy is otherwise correct.
5. **Leaders terminate cleanly.** A leader line must land on the named structure and must not cross multiple plausible targets in a way that makes the label ambiguous.
6. **Trajectories originate correctly.** Needles, scissors, and arrows start at the real entry point and travel in the clinically intended direction. Correct and incorrect comparison paths share the same entry when the procedure logic requires it. Loops, curls, downward exits, paths starting inside the target organ, and random tool/device paths are failures.
7. **Canvas shape matters.** A PASS candidate should be true 4:3 for the app card. A near-square render can be clinically reviewable only as a draft needing crop or light re-render before bundling.
...
### pericardiocentesis_needle_path — Pericardiocentesis, landmark
Ground truth: procedural access geometry, not a routine TTE teaching image. Preferred composition is a reference-style procedural plate: anterior torso with semi-transparent ribs/sternum/costal margins, real syringe/needle entering the subxiphoid/substernal region, and a magnified cardiac inset showing the needle tip in the blue pericardial fluid pocket. The view orientation must be explicit in the composition: in an anterior-facing torso view, the patient's right side is image-left and the patient's left shoulder direction is image-right/superior. Show only the subxiphoid/substernal approach for this asset, not parasternal/apical comparison routes. Needle path / approach angle is the dominant teaching element: shallow, under the costal margin, toward the patient's left shoulder, with the inset tip visibly terminating inside pericardial fluid rather than on epicardium or myocardium. The main torso may show the external syringe/needle up to the skin entry only, as long as the instrument axis clearly teaches the lower-left/bottom-center to upper-right approach and the inset owns the internal endpoint. In screen coordinates, the visible main instrument should read as a forward-slash direction from lower-left/bottom-center toward upper-right; it must not read as lower-right to upper-left. Myocardium and liver are visible as separate unlabeled danger anatomy so a steep/deep path reads as unsafe. Liver danger anatomy must appear on the patient's right upper abdomen / image-left side in that anterior-facing view, not mirrored to the patient's left, and the visible needle/instrument corridor must remain clearly separate from the liver. This asset should be unlabeled; path and direction should be understood visually.
Auto-fail: image reads primarily as a TTE/subxiphoid probe view rather than the procedure; no anterior torso/procedural instrument context; no useful magnified cardiac inset; multiple approach needles/routes shown in this single-focus asset; needle is visually secondary; main needle points lower-right to upper-left instead of lower-left/bottom-center to upper-right; patient laterality is mirrored; liver is on the wrong anatomical side for the chosen view; no liver danger anatomy; visible instrument/needle path crosses, touches, sits on top of, or appears to enter the liver; no myocardium danger anatomy; steep/deep trajectory appears acceptable; needle starts inside the heart; needle tip reads as touching/entering epicardium or myocardium rather than blue pericardial fluid; any effusion/target mark lands on myocardium or heart wall rather than fluid; needle/tool/catheter/guidewire passes through or in front of the myocardium in the main torso; double needle tip or ghost shaft over the heart; a thick tube substitutes for the needle path; liver/myocardium/basic-anatomy labels clutter the procedural geometry view; direction labels such as "Toward left shoulder"; path labels such as "Shallow needle path"; approach letters such as A/B/C; arrow loops/curls/points downward out of the body; title/caption/subtitle text.

### pericardiocentesis_approach — Pericardiocentesis, confirmation
Ground truth: ultrasound target confirmation. The ultrasound panel is the dominant element and shows pericardial effusion, myocardium, drainage target, and needle tip entering the effusion. The tip is in the fluid pocket, not myocardium. This image verifies target acquisition; it does not need to teach body-surface needle geometry because `pericardiocentesis_needle_path` owns that job.
Auto-fail: image is only an external torso/probe scene with no useful ultrasound target; needle tip penetrates myocardium; effusion is absent or not clearly labeled; drainage target is inside myocardium; title/caption/subtitle text.
