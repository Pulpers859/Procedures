...
3. **Required labels exactly once.** Every label in the asset's `requiredLabels` must appear once, spelled verbatim. Duplicated labels, misspellings, parenthetical expansions, invented captions, titles, subtitles, body text, organ/basic-anatomy labels not required by the asset, or other extra labels are failures, even if the anatomy is otherwise correct.
4. **Leaders terminate cleanly.** A leader line must land on the named structure and must not cross multiple plausible targets in a way that makes the label ambiguous.
5. **Trajectories originate correctly.** Needles, scissors, and arrows start at the real entry point and travel in the clinically intended direction. Correct and incorrect comparison paths share the same entry when the procedure logic requires it. Loops, curls, downward exits, paths starting inside the target organ, and random tool/device paths are failures.
6. **Canvas shape matters.** A PASS candidate should be true 4:3 for the app card. A near-square render can be clinically reviewable only as a draft needing crop or light re-render before bundling.
...
Ground truth: procedural access geometry, not a routine TTE teaching image. Oblique torso/procedure view with xiphoid/costal margin, low subxiphoid probe, and one thin needle shaft entering adjacent to the probe. The view orientation must be explicit in the composition: in an anterior-facing torso view, the patient's right side is image-left and the patient's left shoulder direction is image-right/superior. Needle path is the dominant teaching element: shallow, under the costal margin, toward the patient's left shoulder, with the needle tip visibly terminating inside the pericardial effusion/fluid pocket rather than on epicardium or myocardium. Myocardium and liver are visible as separate unlabeled danger anatomy so a steep/deep path reads as unsafe. Liver danger anatomy must appear on the patient's right upper abdomen / image-left side in that anterior-facing view, not mirrored to the patient's left. A small ultrasound inset is acceptable only as confirmation. The label set should stay focused on procedural decision points, not obvious organs.
Auto-fail: image reads primarily as a TTE/subxiphoid probe view rather than the procedure; needle is visually secondary; patient laterality is mirrored; liver is on the wrong anatomical side for the chosen view; no liver danger anatomy; no myocardium danger anatomy; steep/deep trajectory appears acceptable; needle starts inside the heart; needle tip reads as touching/entering epicardium or myocardium rather than blue pericardial fluid; `Pericardial effusion` leader lands on myocardium or heart wall rather than fluid; needle/tool/catheter/guidewire passes through or in front of the myocardium; a thick tube substitutes for the needle path; liver/myocardium/basic-anatomy labels clutter the procedural geometry view; arrow loops/curls/points downward out of the body; title/caption/subtitle text.
