...
| --- | --- | --- | --- |
| `pericardiocentesis_needle_path` | `docs/visual-assets/review-candidates/pericardiocentesis_needle_path_iter32_controlled_schematic.png` | PASS for Patrick review | Controlled schematic generated by `scripts/create_pericardiocentesis_controlled_schematic.py` after Antigravity failed to preserve liver clearance and single-needle geometry. Main torso shows external approach angle only: one continuous syringe/needle enters from lower-left/bottom-center to the subxiphoid/substernal skin entry and points upper-right toward the patient's left shoulder. No internal main-torso needle line crosses the heart. Liver remains image-left and clear of the instrument. Inset shows the needle tip in blue pericardial fluid outside myocardium. Schematic style needs Patrick's taste review before bundling. |
