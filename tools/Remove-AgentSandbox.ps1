...
function ConvertTo-Slug {
    ...
    if ([string]::IsNullOrWhiteSpace($slug)) {
        throw 'The sandbox name must contain at least one letter or number.'
    }
    return $slug
}
...
if (-not (Test-Path -LiteralPath $sandboxPath)) {
    throw "No sandbox was found at $sandboxPath."
}

$reportedRoot = (& git -C $sandboxPath rev-parse --show-toplevel).Trim()
if ($LASTEXITCODE -ne 0) {
    throw "$sandboxPath is not a git worktree."
}
...
if ($status -and -not $Force) {
    throw "Sandbox '$sandboxPath' has local changes. Review them first or rerun with -Force."
}
